...
(function( $ ) {
    ...
    $.ec.modk.ScalarMultiplication = function() {
        ...
        this.keyInput = $( "input[name='random-k']" );
        ...
        // this.randomKWarning = $( "random-k-warning" );
        ...
        this.keyInput.change(function() {
                // mistake = false;
                var k = parseInt($(this).val(), 10);
                var subgroupOrder = parseInt($(".subgroup-order").text(), 10);

                if (k < 1 || k > (subgroupOrder - 1)) {
                    if (k < 1) {
                        k = 1;
                    } else {
                        k = subgroupOrder - 1;
                    ...
                    $(this).val(k);
                    curve.update();
                    // this.randomKWarning.css( "display",
                    //               mistake ? "block" : "none" );
                }
            });
    };
    ...
    $.ec.modk.ScalarMultiplication.prototype.recalculate = function() {
        ...
        //подсчет кратных точек
        this.Pk = this.mulPoint( this.key, this.p);
        this.pkxInput.val(this.Pk[0]);
        this.pkyInput.val(this.Pk[1]);
        this.Qk = this.mulPoint( this.key, this.q);
        this.qkxInput.val(this.Qk[0]);
        this.qkyInput.val(this.Qk[1]);
        $.ec.modk.Base.prototype.recalculate.call( this );
    };
    ...
    $.ec.modk.Encryption  = function()  {
        ...
        // перевод сообщеня пользователя в ASKII
        // this.getASCII = function() {
        //     var mes = this.mesInput.val();
        //     var ascii = [];
        //     for (var i = 0; i < mes.length; i++) {
        //         ascii.push(mes.charCodeAt(i));
        //     }
        //     return ascii;
        // };
        //преобразуем сообщение в ASKII
        // mesASCII  = this.getASCII();
        ...
        // Перевод сообщения в числа по выбранному словарю
        this.getNumbers = function () {
            var alphabet = this.getAlphabet();
            var mes = this.mesInput.val();
            var numbers = [];
            for (var i = 0; i < mes.length; i++) {
                numbers.push(alphabet.get(mes[i]));
            }
            console.log(numbers);
            return numbers;
        };
         // Преобразуем сообщение в числа
        mesNumbers = this.getNumbers();
        //получаем x точки Q
        var xq = this.qkxInput.val();

        var module = this.pInput.val();
        //шифруем
        result = encrypt(mesNumbers, xq, module);
        //размещаем результат шифрования
        document.querySelector("textarea[name='enc-result']").value  = result.join("");
    };

    getRandomK = function()  {
        subgroupOrder = $( ".subgroup-order" );
        var Subgroup = parseInt(subgroupOrder.text(), 10);
        min = 1;
        max = Math.floor(Subgroup-1);
        RandomK = Math.floor(Math.random() * (max - min + 1)) + min;
        keyInput = $( "input[name='random-k']" );
        keyInput.val( RandomK );
        return true
    }

    function encrypt(mesNumbers, xq, module) {
        // Инициализация зашифрованного массива
        const encryptedMes= [];
        // Цикл по элементам mesASCII
        for (let i = 0; i < mesNumbers.length; i++) {
          // Вычисление зашифрованного значения
          const c = (mesNumbers[i] * xq) % module;
            // Преобразование c в строку
            let cString = c.toString();
            // Если длина cString меньше порядка модуля, добавляем ведущие нули
            while (cString.length < module.toString().length) {
            cString = "0" + cString;
            }
            // Добавление зашифрованного значения в массив
            encryptedMes.push(cString);
        }
        // Возврат зашифрованного массива
        console.log("result: ", encryptedMes);
        return encryptedMes;
      }

    $.ec.modk.Encryption.prototype = Object.create( $.ec.modk.Base.prototype );
    $.ec.modk.Encryption.prototype.constructor = $.ec.modk.Encryption;
